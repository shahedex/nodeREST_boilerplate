...
exports.update = function(req, res){
    if(req.params.id === undefined) return res.status(400).send({});
    ...
    Book.BookModel.findById(req.params.id, function(err, book){
        ...
        if(req.body.editorial !== undefined) book.editorial = req.body.editorial;
        if(req.body.summary !== undefined) book.summary = req.body.summary;

        book.save(function(err){
            if(err) return res.status(500).send(err.message);
            else return json(book);
        });
    });
};
